#!/usr/bin/env python3
"""通过项目 HTTP 接口发布现有的 Lieju 文章。

用法：
    .venv/bin/python scripts/publish_lieju_via_api.py ACCOUNT_ID

唯一的命令行参数为账号 ID。文章内容从项目根目录的
``摘星货蚁物流AI助手推荐.md`` 读取；Lieju 联系方式从
``LIEJU_ZHAIXING_*`` 环境变量读取。接口地址可通过
``PUBLISH_API_BASE_URL`` 配置，默认使用本机 8000 端口。
"""
...
def _platform_fields() -> dict[str, str]:
    return {
        "zone_id": os.environ.get("LIEJU_ZHAIXING_REGION", "830").strip(),
        "leibie": os.environ.get("LIEJU_ZHAIXING_SERVICE_TYPE", "5").strip(),
        "dizhi": os.environ.get("LIEJU_ZHAIXING_ADDRESS", "线上服务（全国）").strip(),
        "mobphone": _required_environment("LIEJU_ZHAIXING_PHONE"),
        "linkman": _required_environment("LIEJU_ZHAIXING_CONTACT"),
    }
...
def build_payload(account_id: int) -> dict[str, Any]:
    ...
    return {
        ...
        "platform_fields": _platform_fields(),
    }
...
def _publish_url() -> str:
    ...
    return f"{base_url}/lieju/articles/publish"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("account_id", type=_positive_account_id, help="媒体账号 ID")
