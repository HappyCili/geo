...
请求体包含 `account_id`、`title`、`content` 和 `content_type`；Hepan 与 Lieju 还需要
`category`，CNBlogs 可省略该字段。
`content_type` 默认值为 `markdown`，支持 `markdown` 与 `html`。

## 处理流程

1. 异步 session 查询启用且未删除的媒体账号；CNBlogs 登录态失效时按需刷新。
2. 在公共层解析 `cookie` 或 `cookies` JSON；Hepan、Lieju 和显式传入分类的 CNBlogs
   请求查询平台分类映射。
...
映射至远端分类 ID。Hepan 使用单个 ID；CNBlogs 在显式传入分类时使用逗号分隔的 ID
列表，省略分类时提交空分类；Lieju 使用完整分类路径和 `city_id/fid`。
